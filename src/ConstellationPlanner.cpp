...
ConstellationPlan planOne(const std::string& base,
  ...
  struct Item { std::string label; int len; };
  std::vector<Item> items; items.reserve(variants.size() - 1);
  ...
  for (const auto& it : items) {
    Ray r{ it.label, it.len, toUp ? DiagonalType::Slash : DiagonalType::Backslash };
    if (toUp) out.upRays.push_back(r); else out.downRays.push_back(r);
    toUp = !toUp;
  }
